<template>
  <div class="table-box">
    ...
    <Table
      ...
      :load-data="
        (item, callback) => {
          dealEvent(options.loadData, item, callback)
        }
      "
      @on-current-change="
        (currentRow, oldCurrentRow) => {
          dealEvent(options.onCurrentChange, currentRow, oldCurrentRow)
        }
      "
      @on-drag-drop="
        (index1, index2) => {
          dealEvent(options.onDragDrop, index1, index2)
        }
      "
      @on-select="
        (selection, row) => {
          dealEvent(options.onSelect, selection, row)
        }
      "
      @on-select-cancel="
        (selection, row) => {
          dealEvent(options.onSelectCancel, selection, row)
        }
      "
      @on-select-all="
        (selection) => {
          dealEvent(options.onSelectAll, selection)
        }
      "
      @on-select-all-cancel="
        (selection) => {
          dealEvent(options.onSelectAllCancel, selection)
        }
      "
      @on-selection-change="
        (selection) => {
          dealEvent(options.onSelectionChange, selection)
        }
      "
      @on-sort-change="
        ({ column, key, order }) => {
          dealSortChangeEvent(options.onSortChange, column, key, order)
        }
      "
      @on-filter-change="
        (column) => {
          dealEvent(options.onFilterChange, column)
        }
      "
      @on-row-click="
        (row, index) => {
          dealEvent(options.onRowClick, row, index)
        }
      "
      @on-row-dblclick="
        (row, index) => {
          dealEvent(options.onRowDblclick, row, index)
        }
      "
      @on-expand="
        (row, index) => {
          dealEvent(options.onExpand, row, index)
        }
      "
    >
      <template
