<template>
  <div class="">
    <Card title="示例">
      <div class="tab-wrap">
        <ButtonGroup>
          <Button size="large" v-for="(item, index) in list" :key="index"
            :type="item.routeName === routeName ? 'primary' : 'default'" @click="toPage(item.routeName)">{{item.title}}</Button>
        </ButtonGroup>
      </div>
      <router-view />
    </Card>
  ...
  data () {
    return {
      list: [
        ...
        {
          title: 'tow-row',
          routeName: 'search-table-field-two-row'
        }
      ]
    }
  },
